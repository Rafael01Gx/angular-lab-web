import { Component, OnInit, PLATFORM_ID, TransferState, computed, inject, makeStateKey, signal } from '@angular/core';
...
import { AmostraAnaliseExterna, DashboardCompleto, LaboratorioDashboard, RemessaStatsDashboard } from '../../../shared/interfaces/amostra-analise-externa.interfaces';
import { AmostraLabExternoService } from '../../../services/amostras-analises-externas.service';
import { tap } from 'rxjs';

const DATA_AMOSTRA_KEY = makeStateKey<DashboardCompleto>('analytics-dashboard-data')
@Component({
  ...
  template: `
    <div class="h-full w-full gap-2 bg-gradient-to-b from-blue-50/70 to-slate-50/50 p-0 rounded-sm flex flex-col overflow-hidden">
      <!-- Header -->
      <div class=" p-6 bg-white rounded-md">
        <div class="flex items-center justify-between">
          <div class="flex gap-2">
            <ng-icon name="heroBeaker" class="text-lg text-gray-600" size="24"></ng-icon>
      ...
      <div class="bg-white rounded-2xl shadow-lg p-6 border border-slate-200">
        <div class="flex items-center gap-3 mb-4">
          <ng-icon name="heroFunnel" class="text-indigo-600" size="18"></ng-icon>
          <h2 class="text-md font-semibold text-slate-800">Filtros</h2>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-2">Laboratório</label>
            <select
              [ngModel]="selectedLab()"
              ...
              @for(lab of laboratorios(); track lab.id){<option  [value]="lab.id">{{ lab.nome }}</option>}
            </select>
          </div>
          <div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-gradient-to-br from-blue-600/80 to-blue-700/80 rounded-md shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
        ...
        <div class="bg-gradient-to-br from-green-500/80 to-green-600/80 rounded-md shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
          <div class="flex items-center justify-between mb-4">
            <ng-icon name="heroCheckCircle" size="32" class="opacity-80"></ng-icon>
            <div class="text-3xl font-bold">{{ amostrasCompletas() }}</div>
          </div>
          <h3 class="text-lg font-semibold opacity-90">Análises Concluídas</h3>
          <p class="text-sm opacity-75 mt-1">{{ percentualCompletas() }}% do total</p>
        </div>

        <div class="bg-gradient-to-br from-orange-500/80 to-orange-600/80 rounded-md shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
          <div class="flex items-center justify-between mb-4">
          ...
          <p class="text-sm opacity-75 mt-1">{{ percentualIncompletas() }}% do total</p>
        </div>

        <div class="bg-gradient-to-br from-purple-500/80 to-purple-600/80 rounded-md shadow-lg p-6 text-white transform hover:scale-105 transition-transform">
          <div class="flex items-center justify-between mb-4">
            <ng-icon name="heroArrowTrendingUp" size="32" class="opacity-80"></ng-icon>
            <div class="text-3xl font-bold">{{ mediaElementosPorAmostra() }}</div>
          </div>
      ...
      <div class="flex-1 flex min-h-0">
        ...
        <div class="flex-1 bg-white rounded-md shadow-md p-4 mb-2 border border-slate-200 min-h-0">
        <h2 class="text-md font-bold text-slate-800 mb-6 flex items-center gap-3">
          <ng-icon name="heroBeaker" class="text-indigo-600" size="18"></ng-icon>
          Análise por Laboratório
        </h2>
          ...
          <table class="w-full">
            <thead class="text-center bg-gray-50 sticky top-0 z-10">
              <tr class="border-b-2 border-slate-200">
                <th class="text-left py-4 px-4 font-semibold text-slate-700">Laboratório</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Total</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Concluídas</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Pendentes</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Taxa de Conclusão</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Performance</th>
              </tr>
            </thead>
            <tbody>
              @for(lab of laboratorios(); track lab.id){<tr class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                <td class="py-4 px-4">
                  <div class="font-medium text-slate-800">{{ lab.nome }}</div>
                </td>
                <td class="text-center py-4 px-4 text-slate-600">{{ lab.totalAmostras }}</td>
                <td class="text-center py-4 px-4">
                  <span class="inline-flex items-center gap-1 px-3 py-1 bg-green-100 text-green-700 rounded-full font-medium">
                    <ng-icon name="heroCheckCircle" size="16"></ng-icon>
                    {{ lab.amostrasCompletas }}
                  </span>
                </td>
                <td class="text-center py-4 px-4">
                  <span class="inline-flex items-center gap-1 px-3 py-1 bg-orange-100 text-orange-700 rounded-full font-medium">
                    <ng-icon name="heroXCircle" size="16"></ng-icon>
                    {{ lab.amostrasIncompletas }}
                  </span>
                </td>
                <td class="text-center py-4 px-4">
                  <span class="font-semibold text-slate-700">{{ lab.taxaConclusao }}%</span>
                </td>
                <td class="py-4 px-4">
                  <div class="w-full bg-slate-200 rounded-full h-3 overflow-hidden">
                    <div
                      ...
                      [ngClass]="{
                        ...
                        'bg-yellow-500': lab.taxaConclusao >= 50 && lab.taxaConclusao < 80,
        ...
        <!-- Análise por Remessa -->
        <div class="flex-1 flex flex-col bg-white rounded-md shadow-md p-4 mb-2 border border-slate-200 min-h-0">
        <h2 class="text-md font-bold text-slate-800 mb-6 flex items-center gap-3">
          <ng-icon name="heroCalendar" class="text-indigo-600" size="18"></ng-icon>
          Análise por Remessa
        </h2>
        <div class="flex-1 overflow-y-auto">
          <table class="w-full">
            <thead class="text-center bg-gray-50 sticky top-0 z-10">
              <tr class="border-b-2 border-slate-200">
                <th class="text-left py-4 px-4 font-semibold text-slate-700">Data</th>
                <th class="text-left py-4 px-4 font-semibold text-slate-700">Laboratório</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Total</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Concluídas</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Pendentes</th>
                <th class="text-center py-4 px-4 font-semibold text-slate-700">Taxa</th>
              </tr>
            </thead>
            <tbody>
              @for(remessa of remessasStats(); track $index){<tr  class="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                <td class="py-4 px-4">
                  <span class="inline-flex items-center gap-2 text-slate-700 font-medium">
                    <ng-icon name="heroCalendar" size="16" class="text-indigo-600"></ng-icon>
                    {{ formatDate(remessa.data) }}
                  </span>
                </td>
                <td class="py-4 px-4 text-slate-800">{{ remessa.laboratorio }}</td>
                <td class="text-center py-4 px-4 text-slate-600">{{ remessa.totalAmostras }}</td>
                <td class="text-center py-4 px-4">
                ...
                <td class="text-center py-4 px-4 font-semibold text-slate-700">{{ remessa.taxaConclusao }}%</td>
              </tr>}
            </tbody>
          </table>
        </div>
...
export class LabExternoDashboardComponent implements OnInit {
  ...
  mediaElementosPorAmostra = computed(() => {
    const total = this.filteredAmostras().reduce((acc, a) =>
      acc + a.elementosSolicitados.length, 0
    );
  ...
  laboratorios = computed(() => {
    ...
    labs.forEach(lab => {
      lab.taxaConclusao = lab.totalAmostras > 0
        ? Math.round((lab.amostrasCompletas / lab.totalAmostras) * 100)
        : 0;
    });

    return Array.from(labs.values()).sort((a, b) =>
      b.totalAmostras - a.totalAmostras
    );
  });
  ...
  remessasStats = computed(() => {
    ...
    remessas.forEach(remessa => {
      remessa.taxaConclusao = remessa.totalAmostras > 0
        ? Math.round((remessa.amostrasCompletas / remessa.totalAmostras) * 100)
        : 0;
    });

    return Array.from(remessas.values()).sort((a, b) =>
  ...
  loadData() {
    this.#amostrasAnalisesExternasService.getDashboardCompleto().pipe(tap((data) => {
  ...
  applyFilters() {
    ...
    if (this.selectedLab()) {
      filtered = filtered.filter(a =>
        a.RemessaLabExterno.destino.id === parseInt(this.selectedLab())
      );
    }

    if (this.dateStart()) {
      filtered = filtered.filter(a =>
        a.RemessaLabExterno.data >= this.dateStart()
      );
    }

    if (this.dateEnd()) {
      filtered = filtered.filter(a =>
        a.RemessaLabExterno.data <= this.dateEnd()
      );
    }
  ...
  exportToExcel() {
    ...
    const overview = [
      ...
      ['Período ', this.dateStart() && this.dateEnd() ? (new Date(this.dateStart()).toLocaleString('pt-BR').split(",")[0] + " até " + new Date(this.dateEnd()).toLocaleString('pt-BR').split(",")[0]) : ""],
      [],
      ['Métricas Gerais'],
    ...
    XLSX.writeFile(wb, `relatorio-analises-laboratoriais-${new Date().toISOString().split('T')[0]}.xlsx`);
  }
  ...
  exportToPDF() {
    ...
    if (this.dateStart() && this.dateEnd()) {
      doc.text(`Período - ${new Date(this.dateStart()).toLocaleString('pt-BR').split(",")[0]} até ${new Date(this.dateEnd()).toLocaleString('pt-BR').split(",")[0]} `, 14, 32);
    }
    ...
    const metricsData = [
      ...
      ['Percentual Concluído', `${this.percentualCompletas()}%`],
      ['Média de Elementos/Amostra', this.mediaElementosPorAmostra().toString()]
    ];
    ...
    autoTable(doc, {
      startY: 24,
      head: [['Data', 'Laboratório', 'Total', 'Concluídas', 'Pendentes', 'Taxa']],
      body: remessaData,
      theme: 'grid',
      headStyles: { fillColor: [79, 70, 229] },
      styles: { fontSize: 8 }
    ...
    doc.save(`relatorio-analises-laboratoriais-${new Date().toISOString().split('T')[0]}.pdf`);
  }
