...
@Component({
  ...
  template: `
    <div
      class="h-full w-full gap-1 bg-white p-0 rounded-sm flex flex-col overflow-hidden"
    >
      <!-- Header -->
      <div
        class="relative flex items-center gap-2 bg-gradient-to-r from-blue-600/80 to-blue-700/80 px-6 py-4 text-white"
      >
        <div
          (click)="voltar()"
          class="absolute top-0 left-0 h-full flex items-center shadow-lg rounded-l-md border-r hover:cursor-pointer"
        >
          <ng-icon
            name="heroChevronLeft"
            class="text-slate-400  hover:scale-107 "
            size="24"
          />
        </div>
        <h2 class="text-xl font-semibold ml-4">
          Incluir Resultados de Analíses
        </h2>
      </div>
      ...
      <div class="flex-1 flex-col overflow-y-auto">
          ...
          <div class="flex gap-4 items-start min-h-0">
            ...
            <div class="flex-1">
              ...
              >
                ...
                <p class="text-slate-600 font-medium mb-1">
                  Clique para selecionar arquivo de resultados de análises
                </p>
            ...
            >
              <ng-icon
                ...
                [class.animate-spin]="
                  showConfig() && !selectedFile() && !(amostras().length > 0)
                "
              ></ng-icon>
            </button>
          </div>
        ...
        <div
          class="flex-1 flex flex-col mt-6 bg-slate-50 rounded-md p-6 border border-slate-200 overflow-hidden"
        >
            ...
            <div class="flex flex-wrap gap-4 pb-6">
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span class="text-xs text-slate-500 block font-normal"
                    >Índice da coluna onde está o valor "Type"</span
                  >
                </label>
                <input
                  type="number"
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  Valor do cabeçalho
                  <span class="text-xs text-slate-500 block font-normal"
                    >Texto esperado na célula de cabeçalho</span
                  >
                </label>
                <input
                  type="text"
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span class="text-xs text-slate-500 block font-normal"
                    >Linhas após cabeçalho onde ficam nomes dos elementos</span
                  >
                </label>
                <input
                  type="number"
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span class="text-xs text-slate-500 block font-normal"
                    >Linhas após elementos onde ficam as unidades</span
                  >
                </label>
                <input
                  type="number"
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span class="text-xs text-slate-500 block font-normal"
                    >Primeira coluna onde começam os elementos químicos</span
                  >
                </label>
                <input
                  type="number"
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span class="text-xs text-slate-500 block font-normal"
                    >Valor que identifica linha como amostra (ex: SMP)</span
                  >
                </label>
                <input
                  type="text"
              ...
              <div class="flex-[calc(50%-0.5rem)]">
                <label class="block text-sm font-medium text-slate-700 mb-1">
                  ...
                  <span class="text-xs text-slate-500 block font-normal"
                    >Distância da coluna Sample ID em relação ao cabeçalho</span
                  >
                </label>
                <input
                  type="text"
        ...
        <div class="p-6 overflow-y-auto">
          ...
          <div class="space-y-4">
            ...
            <div
              class="bg-white border border-slate-200 rounded-md overflow-hidden shadow-sm"
            >
                ...
                >
                  ...
                  <div class="flex flex-col">
                    ...
                    <div class="flex items-center gap-2">
                      ...
                      />
                      <span
                        class="text-sm font-medium text-slate-600 min-w-[50px]"
                      >
                        {{ elemento.unidade }}
                      </span>
                    </div>
      ...
      <div class="w-full p-4 flex justify-end shadow-sm gap-4">
        ...
        >
          ...
          Salvar Selecionadas ({{ selectedAmostras().size }})</button
        >}
      </div>
      }
    </div>
...
export class UploadResultadoComponent {
  ...
  handleUpload(): void {
    ...
    if (this.config().hasValuesConfig.length > 0) {
      this.config().hasValuesConfig = (this.config().hasValuesConfig as string)
        ?.split(',')
        .map((item) => item.trim());
    }
  ...
  handleSubmitSelected(): void {
    ...
    const data = selectedData.map((a) => {
      a.analiseConcluida = true;
      return a;
    });
